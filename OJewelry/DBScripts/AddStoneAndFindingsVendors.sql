...
SELECT 'FV_'+name ,id, 4 
FROM Companies as c
WHERE NOT EXISTS (SELECT * from Vendors as v where v.CompanyId = c.id and v.Type_Type = 4)  
UNION
SELECT 'SV_'+name, id, 2 
FROM Companies as c
WHERE NOT EXISTS (SELECT * from Vendors as v where v.CompanyId = c.id and v.Type_Type = 2)  
commit transaction
...
where vendorId is null and v.Type_Type = 2
select * from findings f
join vendors v on v.CompanyId = f.CompanyId 
where vendorId is null and v.Type_Type = 4
